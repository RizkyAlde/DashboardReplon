...
const TimeSeries = ({ gh }) => {
  ...
  const timeLabels =
    data?.data_sensor.map((sensor) => sensor.time).reverse() || [];
  const humidityData =
    data?.data_sensor.map((sensor) => sensor.humid).reverse() || [];
  const lumenData =
    data?.data_sensor.map((sensor) => sensor.lumen).reverse() || [];
  const soilData =
    data?.data_sensor.map((sensor) => sensor.soil).reverse() || [];
  const temperatureData =
    data?.data_sensor.map((sensor) => sensor.temp).reverse() || [];
