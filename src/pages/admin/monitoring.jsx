...
const Monitoring = () => {
    ...
    const { data: pumpData, isLoading: pumpLoading, error: pumpError } = usePumpGH(selectedType);

    const handleOnChange = (e) => {
        setSelectedType(e.target.value); // Update the selected type
    };

    if (isLoading || pumpLoading) return <p className="flex justify-center items-center">Loading...</p>;

    if (error) return <p className="flex justify-center items-center">Error: {error.message}</p>;

    if (pumpError) return <p className="flex justify-center items-center">Error: {pumpError.message}</p>;
        ...
        <div className="grid w-full grid-cols-1 justify-center">
            ...
            <div className="flex gap-4 flex-col lg:flex-row">
                <div className="p-6 lg:w-1/3 w-full bg-white border border-gray-200 rounded-lg shadow mb-4">
                    <PumpStatus pumpStatus={pumpData?.status_pompa ? "ON" : "OFF"} title="Status Pompa"/>
                </div>
